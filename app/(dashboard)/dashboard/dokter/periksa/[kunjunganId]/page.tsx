...
export default async function PeriksaPage({ params }: PeriksaPageProps) {
  ...
  if (!result.success || !result.data) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] p-8 bg-card rounded-2xl border border-red-100 dark:border-red-800 shadow-sm my-6">
        <div className="w-12 h-12 flex items-center justify-center rounded-full bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-300 mb-4">
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
        </div>
        <h3 className="text-lg font-bold text-foreground mb-2">Gagal Memuat Halaman Pemeriksaan</h3>
        <p className="text-sm text-muted-foreground mb-2 text-center max-w-md">
          Terjadi kesalahan saat mengambil detail kunjungan.
        </p>
        <div className="bg-red-50 dark:bg-red-900/20/50 border border-red-100 dark:border-red-800 rounded-lg p-3 text-xs font-mono text-red-700 dark:text-red-300 mb-6 max-w-lg w-full text-center">
